...
/**
 * @Time 2022-11-04 11:24
 * Created by Huang
 * className: ChannelFileZipUtil
 * Description:
 */
public class ChannelFileZipUtil {
    /**
     * 压缩成ZIP 方法1
     *
    ...
    private static void compressFolder(File sourceDir, ZipOutputStream zipOut, String name, boolean keepDirStructure) throws Exception {
        ...
        if (sourceDir.isFile()) {
            ...
            // copy文件到zip输出流中
            //FileChannel
            FileChannel fileChannel = new FileInputStream(sourceDir).getChannel();
            fileChannel.transferTo(0,sourceDir.length(), writableByteChannel);
            // Complete the entry
            zipOut.closeEntry();
        } else {//如果是文件夹
    ...
    public static void main(final String[] args) {
        zipFolder("H:\\personal\\project\\huang-learn-summary\\zip\\folder", "H:\\personal\\project\\huang-learn-summary\\zip\\folder_channel.zip", true);
        //1G 耗时：42322ms
    }
}
