...
/**
 * @Time 2022-11-04 11:13
 * Created by Huang
 * className: BufferedFileZipUtil
 * Description:
 */
public class BufferedFileZipUtil {
    /**
     * 压缩成ZIP 方法1
     *
    ...
    private static void compressFolder(File sourceDir, ZipOutputStream zipOut, String name, boolean keepDirStructure) throws Exception {
        //如果是文件
        if (sourceDir.isFile()) {
            //更换成BufferedOutputStream
            BufferedOutputStream bufferedOutputStream = new BufferedOutputStream(zipOut);
            // 向zip输出流中添加一个zip实体，构造器中name为zip实体的文件的名字
            zipOut.putNextEntry(new ZipEntry(name));
            // copy文件到zip输出流中
            int temp;
            //更换成BufferedInputStream
    ...
    public static void main(final String[] args) {
        zipFolder("H:\\personal\\project\\huang-learn-summary\\zip\\folder", "H:\\personal\\project\\huang-learn-summary\\zip\\folder_buffered.zip", true);
        //1G 耗时：50095ms
    }
}
